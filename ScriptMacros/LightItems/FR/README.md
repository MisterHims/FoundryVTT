...
1. Copiez le code ci-dessous ou accédez-y depuis la [Collection](https://github.com/MisterHims/FoundryVTT/blob/master/ScriptMacros/LumenVision/FR/Collection/Lampe.js) sous le nom de "Lampe.js" :
   ...
   t item = actor.items.find(i => i.name === "Huile");
   nst macro = game.macros.entities.find(m => m.name === "consum-generic");
   (!macro) {
   .notifications.error("Cette macro dépends de la macro 'consum-generic' qui ne peut être trouvée.");
   return;
